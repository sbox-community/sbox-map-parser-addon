...
// Adapted for s&box; https://github.com/sbox-community/sbox-utils
...
#region Public fields
        /// <summary>True if UTF8 encoding for filename and comments, false if default (CP 437)</summary>
        public bool EncodeUTF8 = true; // CP 437 is not exists as default
...
#region Private fields
		...
		private static Encoding DefaultEncoding = Encoding.UTF8; //Encoding.GetEncoding(437); // CP 437 is not exists as default
#endregion
...
#region Public methods
        ...
        /// </summary>
        /// <param name="_filesystem">Where is your files like FileSystem.Data for data/your_gamemode/ folder</param>
        /// <param name="_filename">Full path of Zip file to create</param>
        /// <param name="_comment">General comment for Zip file</param>
        /// <returns>A valid ZipStorer object</returns>
        public static ZipStorer Create(BaseFileSystem _filesystem, string _filename, string _comment = null)
        {
			Stream stream = _filesystem.OpenWrite( _filename, FileMode.OpenOrCreate );//new FileStream(_filename, FileMode.Create, FileAccess.ReadWrite);

            ZipStorer zip = Create(stream, _comment);
            zip.Comment = _comment ?? string.Empty;
            zip.FileName = _filename;
            zip.FileSystem = _filesystem;
		...
		/// <returns>A valid ZipStorer object</returns>
		public static ZipStorer Open(BaseFileSystem _filesystem, string _filename, FileMode _access )
		{
            Stream stream = null;
            ZipStorer zip = null;
            try
            {
				stream = _filesystem.OpenRead( _filename, _access == FileMode.Open ? FileMode.Open : FileMode.OpenOrCreate ); //(Stream)new FileStream(_filename, FileMode.Open, _access == FileAccess.Read ? FileAccess.Read : FileAccess.ReadWrite);
        ...
        /// <param name="_leaveOpen">true to leave the stream open after the ZipStorer object is disposed; otherwise, false (default).</param>
        /// <returns>A valid ZipStorer object</returns>
        public static ZipStorer Open(Stream _stream, FileMode _access, bool _leaveOpen = false)
        {
            if (!_stream.CanSeek && _access != FileMode.Open)
            ...
            //throw new System.IO.InvalidDataException(); (WHITELIST)
            throw new Exception();
        }
        ...
        {
            if (Access == FileMode.Open)
                throw new InvalidOperationException("Writing is not alowed");

            using (var stream = this.FileSystem.OpenRead( _pathname )) //new FileStream(_pathname, FileMode.Open, FileAccess.Read)
			{
                return this.AddStream(_method, _filenameInZip, stream, DateTime.Now, _comment); //File.GetLastWriteTime(_pathname) (WHITELIST)
			}
        }
...
#else            
            return GameTask.RunInThreadAsync( () => this.AddStreamAsync(_method, _filenameInZip, _source, _modTime, _comment)).Result;
#endif
        }
		...
		public void AddDirectory( Compression _method, string _pathname, string _pathnameInZip, string _comment = null )
		{
			...
			string[] fileEntries = this.FileSystem.FindFile( _pathname ).ToArray();//Directory.GetFiles( _pathname );

			foreach ( string fileName in fileEntries )
				this.AddFile( _method, fileName, foldername + Path.GetFileName( fileName ), "" );

			// Recurse into subdirectories of this directory.
			string[] subdirectoryEntries = this.FileSystem.FindDirectory( _pathname ).ToArray();//Directory.GetDirectories( _pathname );
		...
		public void Close()
		{
			if ( this.Access != FileMode.Open )
			{
				...
				if ( this.CentralDirImage != null )
					this.WriteEndRecord( centralSize + (uint)CentralDirImage.Length, centralOffset, Files.Count == 0 );
				else
					this.WriteEndRecord( centralSize, centralOffset, Files.Count == 0 );
			}
        ...
        public bool ExtractFile( ZipFileEntry _zfe, string _filename)
        {
            ...
            if ( !string.IsNullOrEmpty( path ) && !this.FileSystem.DirectoryExists(path) )//Directory.Exists(path)
				this.FileSystem.CreateDirectory(path);

			// Check if it is a directory. If so, do nothing.
			if ( this.FileSystem.DirectoryExists( _filename ) )//Directory.Exists(_filename))
				return true;

            bool result;
			using ( var output = this.FileSystem.OpenWrite( _filename ) )//new FileStream(_filename, FileMode.Create, FileAccess.Write))
			{
...
#else                
            return GameTask.RunInThreadAsync(() => ExtractFileAsync(_zfe, _stream)).Result;
#endif
        }
		...
		{
			if ( !(_zip.ZipFileStream is Stream) )
				throw new InvalidOperationException( "RemoveEntries is allowed just over streams of type FileStream" );

			//Get full list of entries
			var fullList = _zip.ReadCentralDir();

			var fs = _zip.FileSystem;

			//In order to delete we need to create a copy of the zip file excluding the selected items
			var tempZipName = $"{Path.GetDirectoryName( _zip.FileName)}{Path.GetFileName( Path.GetTempFileName() )}";//Path.GetTempFileName();
			var tempEntryName = $"{Path.GetDirectoryName( _zip.FileName )}{Path.GetFileName( Path.GetTempFileName() )}";//Path.GetTempFileName();

			try
			{
				var tempZip = ZipStorer.Create( fs, tempZipName, string.Empty );

				foreach ( ZipFileEntry zfe in fullList )
				{
					if ( !_zfes.Any(x=>x.FilenameInZip == zfe.FilenameInZip ))//if ( !_zfes.Contains( zfe ) )
					{
				...
				if ( fs.FileExists( _zip.FileName ) )
					fs.DeleteFile( _zip.FileName );

				var newfile = fs.OpenWrite( _zip.FileName, FileMode.Create );
				var oldfile = fs.OpenRead( tempZipName, FileMode.Open );
				...
				_zip = ZipStorer.Open( fs, _zip.FileName, _zip.Access );
			}
			catch
			{
				return false;
			...
			finally
			{
				...
				if ( fs.FileExists( tempZipName ) )
					fs.DeleteFile( tempZipName );
				if ( fs.FileExists( tempEntryName ) )
					fs.DeleteFile( tempEntryName );
        ...
        */
        private void WriteEndRecord(long _size, long _offset, bool _empty)
        {
			...
			this.ZipFileStream.Write( _empty ? new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 } : new byte[] { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF }, 0, 12);
            this.ZipFileStream.Write(BitConverter.GetBytes((ushort)encodedComment.Length), 0, 2);
            this.ZipFileStream.Write(encodedComment, 0, encodedComment.Length);
        }
