...
namespace TgaDecoderTest
{
    public class TgaDecoder
    {
        ...
        public static TgaData FromFile(BaseFileSystem filesystem, string path )
        {
            try
            {
				var fs = filesystem.ReadAllBytes( path );
				return decode( fs.ToArray() );
			}
            catch (Exception e)
            {
				Log.Info( e.Message );
        ...
        public static TgaData FromBinary(byte[] image) => decode(image);
        protected static TgaData decode(byte[] image) => new TgaData( image );
